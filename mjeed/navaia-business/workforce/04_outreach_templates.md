...
> **Updated 2026-07-13:** Snov.io is live and **auto-appends the account's configured signature**
> to every send (verified: the HTML signature renders as a branded block). So the body carries
> **no signature** — do not embed one or pass `--signature-file` on Snov sends (it double-stamps).
> Canonical signature mirrored at `assets/email_signature.html` for reference.
...
- **Say the automation, every message, both channels.** The solution is named as smart
  automation (حلول ذكية تُؤتمت…) at least once per email AND per WhatsApp (in `{{2}}`),
  so NAVAIA is never mistaken for an agency, call center, or manpower service.
- **Never cite reviews.** Review evidence is INTERNAL — it only picks which pain to lead
  with. The copy states that pain as a known, fixable pain in their line of work; it never
  says or implies we read their reviews/ratings (no "لاحظت أن مراجعيكم ذكروا", no
  "في تقييماتكم", nothing that attributes the pain to feedback about them) — even when the
  pain is that specific to the organization.
...
- **`{trigger_line}`** : filled by the **trust-locked review pass** (`scripts/enrich_reviews.py`):
  ...
  exact CRM lead. The reviews only PICK the pain — the sentence itself states it as a known,
  fixable pain with NO attribution (e.g. "في مثل عملكم قد يتأخّر الردّ على الاتصالات
  والرسائل في أوقات الذروة" — never "لاحظت أن مراجعيكم ذكروا…").
...
5. **Question close** : one question on its own line, then the cal.com link + WhatsApp option
   on the next line.
6. **No signature in the body.** Snov.io **auto-appends the account's configured signature** to
   every send (verified 2026-07-13 — the HTML signature renders as a branded block). Do **not**
   put a signature in the copy or pass `--signature-file` on Snov sends — that double-stamps it.
   The body ends on the question-close line (cal.com link + WhatsApp option). The canonical
   signature is mirrored at `assets/email_signature.html` for reference only.
...
Pain (per the vertical one-pager): most missed calls arrive **in the evening after the clinic
closes** — unanswered, unbooked; cancellations/no-shows for lack of WhatsApp+SMS reminders;
**reception staff turnover** makes service quality unstable. ROI frame: one extra booked
appointment covers the monthly cost. Benefit pair: profit +30% and cost −40%. Compliance: MoH + PDPL.
**Field lexicon:** المراجعين، إشغال الجدول، قائمة الانتظار، عدم الحضور (no-show)، تأكيد المواعيد، المكالمات الفائتة، دوران موظفي الاستقبال.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تُؤتمت الردّ والحجز، ليبقى جدولكم ممتلئاً دون عبء على فريق استقبالكم.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تُؤتمت هذا الجانب، فلا يضيع من وقت فريقكم ما كان للمراجع الحاضر أمامهم.
...
Pain (per the vertical one-pager): most O&M contractors **lose contracts every week** for lack
of a team that monitors tenders and answers in time — manual, intermittent tracking of
**اعتماد (Etimad) وفرصة (Furas)**, late bids because eligibility review is slow; and no
parallel **private-sector contracts line** with facility owners. Decision maker: owner.
Value = a won contract. Benefit pair: profit +30% and cost −40%. Compliance: PDPL + أنظمة العمل.
**Field lexicon:** عروض الأسعار (RFQ)، المناقصات والعطاءات، كراسة الشروط، مواعيد تسليم العطاءات، الترسية، عقود الصيانة الوقائية، SLA، منصة اعتماد، منصة فرصة، تقييم الأهلية.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تُؤتمت متابعة العروض والعطاءات، فلا يفوتكم عقدٌ بسبب تأخّر متابعةٍ أو عرضٍ معلّق.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تُؤتمت هذا الجانب، فلا يبقى عرضٌ لكم معلّقاً ولا عطاءٌ دون متابعة.
...
Pain (per the vertical one-pager): the stuck debt is **dead revenue** (الدَّين المتعثّر إيرادٌ
ميت); manual follow-up arrives late — after the debt ages — and **burns collectors out**
(hiring, training, turnover); compliance is a manual burden when calls aren't documented for
the regulator. Flip: 24/7 early collection before aging, every call documented and provable —
compliance becomes an advantage. Pricing = % of collected. Benefit pair: profit +30% (more
collected) and cost −40%. Compliance: **SAMA/البنك المركزي** + PDPL : lead with it.
**Field lexicon:** المتعثّرات، أعمار الديون (aging)، محفظة التحصيل، نسبة التحصيل، الأقساط المتأخّرة، لوائح ممارسات التحصيل، ضوابط البنك المركزي، الوعد بالدفع، التحصيل المبكّر.
...
نڤايا تعمل إلى جانب فريقكم بحلولٍ ذكية تُؤتمت المتابعة والتذكير، فلا يبقى متعثّرٌ لكم دون متابعةٍ في وقتها.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تُؤتمت هذا الجانب ووفق أنظمة ساما، فيبقى كل متعثّرٍ لكم تحت المتابعة حتى السداد.
...
Pain (per the vertical one-pager): the interested client **goes cold within minutes** if no
one answers — and calls a competitor; sale + rental listings sit with **no outbound campaigns**
from day one; **rent collection** reminders and post-sale owner support need a team that
doesn't exist; **development tenders** (اعتماد وفرصة) pass unexamined. Dual buyer with the
debt agent. Benefit pair: profit +30% (closed deals) and cost −40%.
Compliance: PDPL + الأنظمة العقارية (REGA).
**Field lexicon:** المهتمّ (lead)، المعاينة، الوحدات الشاغرة، نسبة الإشغال، دفعات الإيجار وسنداتها، الحملات الصادرة، قوائم البيع والإيجار، مناقصات التطوير.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تُؤتمت الردّ والمتابعة، فلا يبرد مهتمّكم ولا تبقى وحدتكم شاغرةً بسبب تأخّر الردّ.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تُؤتمت هذا الجانب، فلا تتأخّر دفعةٌ لكم ولا يبقى مستأجرٌ دون متابعة.
...
Pain (per the vertical one-pager): **registration season drowns the team** — calls and
messages missed at the exact peak that pays for the year; enquirers not followed **until
enrolment is decided** (حتى الحسم); and **government training tenders don't wait** — instant
screening on اعتماد وفرصة with the bid ready before the deadline, or the opportunity passes.
One team for both needs. Benefit pair: profit +30% (more enrolments) and cost −40%.
Compliance: PDPL + أنظمة التدريب (TVTC).
**Field lexicon:** المتدربين، القبول والتسجيل، الدفعات والأفواج، المنافسات الحكومية، منصة اعتماد (Etimad)، منصة فرصة، ذروة الموسم، حتى الحسم.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تُؤتمت الردّ والتسجيل، فلا يضيع مستفسرٌ لكم مهما اشتدّ زحام موسمكم.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تُؤتمت هذا الجانب، فلا يتوقّف متدرّبٌ لكم في منتصف تسجيله دون متابعة.
