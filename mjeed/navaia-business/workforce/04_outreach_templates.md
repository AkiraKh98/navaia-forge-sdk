...
- **Say the automation, every message, both channels.** The solution is named as smart
  automation (حلول ذكية تتولّى…) at least once per email AND per WhatsApp (in `{{2}}`),
  so NAVAIA is never mistaken for an agency, call center, or manpower service.
- **Never cite reviews.** *(Enforced structurally since 2026-07-21: `{{2}}` is generated by
  `lina_compose.generate_block`, which receives only English pain descriptors and counts —
  never a review, quote or rating — so the copy CANNOT cite feedback even if the model tried.
  `validate_block` rejects any output containing a review tell. The rule below still governs
  every hand-written line.)*
  Review evidence is INTERNAL — it only picks which pain to lead
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تتولّى الردّ والحجز، ليبقى جدولكم ممتلئاً دون عبء على فريق استقبالكم.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تتولّى متابعة العروض والعطاءات، فلا يفوتكم عقدٌ بسبب تأخّر متابعةٍ أو عرضٍ معلّق.
...
نڤايا تعمل إلى جانب فريقكم بحلولٍ ذكية تتولّى المتابعة والتذكير، فلا يبقى متعثّرٌ لكم دون متابعةٍ في وقتها.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تتولّى الردّ والمتابعة، فلا يبرد مهتمّكم ولا تبقى وحدتكم شاغرةً بسبب تأخّر الردّ.
...
نڤايا تعمل إلى جانبكم بحلولٍ ذكية تتولّى الردّ والتسجيل، فلا يضيع مستفسرٌ لكم مهما اشتدّ زحام موسمكم.
